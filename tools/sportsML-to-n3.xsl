...
select="substring-after(newsml:newsItem/newsml:contentMeta/newsml:subject[newsml:broader/@qcode='medtop:15000000']/@qcode,':')"/>
    ...
    <xsl:template match="/">
        <xsl:choose>
            ...
            <!-- Sports where league is governing body and season is competition -->
            <xsl:when test="$sport-code = ('20000849','20000851','20000965','20000823')">
                <xsl:value-of select="$season-id"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'governingBody','»')"/>~<xsl:value-of select="$competition-id"/> .
            </xsl:when>
            <xsl:when test="/newsml:newsItem/newsml:contentMeta/newsml:subject[@type='spct:competition']">
                <xsl:for-each select="/newsml:newsItem/newsml:contentMeta/newsml:subject[@type='spct:competition']">
                    <xsl:variable name="competition-id" select="concat('«',$sport-vendor-ns,'Competition/',substring-after(@qcode,':'),'»')"/>
                    <xsl:variable name="parent-competition-key" select="newsml:broader/@qcode"/>
                    ...
                    <xsl:if test="newsml:broader and //newsml:subject[@qcode=$parent-competition-key]/@type='spct:competition'">
                        <xsl:variable name="parent-competition-id" select="concat('«',$sport-vendor-ns,'Competition/',substring-after(newsml:broader/@qcode,':'),'»')"/>
                ...
                <xsl:if test="//newsml:subject/@type='spct:governing-body'">
                    <xsl:variable name="governing-body-id" select="concat('«',$sport-vendor-ns,'GoverningBody/',substring-after(//newsml:subject[@type='spct:governing-body']/@qcode,':'),'»')"/>
                    <xsl:value-of select="$governing-body-id"/> <xsl:value-of select="concat('«',$rdf-ns,'type','»')"/> <xsl:value-of select="concat('«',$sport-ontology-ns,'GoverningBody','»')"/> .
                    <xsl:value-of select="$governing-body-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/> "<xsl:value-of select="//newsml:subject[@type='spct:governing-body']/newsml:name"/>" .
                </xsl:if>
            </xsl:when>
            <xsl:otherwise>
            </xsl:otherwise>
    ...
    <xsl:template match="newsml:sports-event">
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of select="$event-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/> "Event <xsl:value-of select="$event-id"/>" .
          </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:if test="ancestor::newsml:tournament-part/newsml:tournament-part-metadata/@type">
            ...
            <xsl:value-of select="$event-id"/>~<xsl:value-of select="concat('«', $sport-ontology-ns, 'competition', '»')"/>~<xsl:value-of select="$phase-id"/> .
            ...
            <xsl:value-of select="$phase-id"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'parentCompetition','»')"/>~<xsl:value-of select="$competition-id"/> .

            <xsl:if test="parent::newsml:tournament-part/newsml:tournament-part-metadata//newsml:name">
    ...
    <xsl:template match="newsml:event-metadata">
        ...
        <xsl:if test="newsml:event-metadata-soccer/@week and not(ancestor::newsml:tournament-part)">
            ...
            <xsl:value-of select="$event-id"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'competition','»')"/>~<xsl:value-of select="$competition-phase-id" /> .
            ...
            <xsl:value-of select="$competition-phase-id"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'parentCompetition','»')"/>~<xsl:value-of select="$season-id"/> .
        </xsl:if>
    ...
    <!-- top-level competition, e.g. league -->
    <xsl:template match="newsml:subject">
        <xsl:choose>
            <xsl:when test="@type='spct:league'">
                <xsl:variable name="competition-name" select="newsml:name"/>
                <xsl:value-of select="$competition-id"/>~<xsl:value-of select="concat('«',$rdf-ns,'type','»')"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'Competition','»')"/> .
                <xsl:value-of select="$competition-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/>~"<xsl:value-of select="$competition-name"/>" .
            </xsl:when>
            <xsl:when test="@type='spct:season'">
                <xsl:variable name="competition-name" select="/newsml:newsItem/newsml:contentMeta/newsml:subject/[@type='spct:league']"/>
                <xsl:variable name="season-name" select="newsml:name"/>
    ...
    <xsl:template match="newsml:player">
        ...
        <xsl:variable name="player-name" select="newsml:player-metadata/newsml:name[@role='nrol:full']"/>
        ...
        <xsl:choose>
            <xsl:when test="string($player-name)">
                <xsl:value-of select="$player-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/>~"<xsl:value-of select="$player-name"/>" .
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="$player-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/>~"Player <xsl:value-of select="$player-id"/>" .
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="newsml:associate">
        ...
        <!-- <associate> sport:positionEvent <associate-position> -->
        <xsl:if test="newsml:associate-metadata/@position-event">
            <xsl:value-of select="$associate-id"/>~<xsl:value-of select="concat('«',$sport-ontology-ns,'positionEvent','»')"/>~<xsl:value-of select="concat('«',$newscode-ns,substring-before(newsml:associate-metadata/@position-event,':'),'/',substring-after(newsml:associate-metadata/@position-event,':'),'»')"/> .
        </xsl:if>
    ...
    <xsl:template match="newsml:official">
        ...
        <xsl:variable name="official-name" select="newsml:official-metadata/newsml:name[@role='nrol:full']"/>
        ...
        <xsl:if test="string($official-name)">
    ...
    <xsl:template match="newsml:action[@id]">
        ...
        <xsl:value-of select="$action-id"/>~<xsl:value-of select="concat('«',$rdfs-ns,'label','»')"/> "Action <xsl:value-of select="$action-id"/> on event <xsl:value-of select="$event-id"/>" .
    ...
    <xsl:template match="newsml:stat">
        ...
        <xsl:if test="@value != '' and @value != 'NaN'">
            ...
            <xsl:variable name="value-name" select="substring-after(@stat-type,':')"/>

            <!-- <participation> newscodescv:cvterm "value" -->
            <xsl:value-of select="$participation-id"/>~<xsl:value-of select="concat('«',$newscode-ns,$cv-name,'/',$value-name,'»')"/>~<xsl:text>"</xsl:text><xsl:value-of select="@value"/><xsl:text>"</xsl:text> .
        </xsl:if>
    </xsl:template>
